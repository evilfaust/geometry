...
const constructions = [
  ...
  {
    name: "Построение угла, равного данному",
    button: "Равный угол",
    buttonText: "Построить угол, равный данному.",
    lead: "Удобнее всего переносить угол не по градусам, а по двум одинаковым дугам и расстоянию между точками на этих дугах.",
    idea: "Сначала на исходном угле отмечают точки P и Q одной дугой. Потом такую же дугу строят из новой вершины A и на ней откладывают расстояние PQ от точки C.",
    proof: "Если дуги имеют одинаковый радиус, а хорда CE равна хорде PQ, то дуги стягивают равные углы. Поэтому угол EAB равен исходному углу.",
    use: "Такой способ нужен в задачах на построение, когда угол надо точно перенести в другую точку без транспортира.",
    stepsCopy: "Сначала отмечаем на исходном угле две точки одной дугой, потом повторяем эту дугу и переносим расстояние между найденными точками.",
    points: {
      K: { x: -1.35, y: -0.1 },
      M: { x: -0.2, y: -0.1 },
      N: { x: -0.72, y: 0.95 },
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Возьми удобный раствор циркуля и проведи дугу с центром в K. Она пересечет стороны угла в точках P и Q.",
        note: "Главное здесь не величина радиуса, а то, что этот же раствор потом повторится из точки A.",
      },
      {
        title: "Шаг 3. Повтори такую же дугу из точки A",
        description: "С тем же радиусом проведи дугу с центром в A. Она пересечет луч AB в точке C. Теперь возьми раствор циркуля по отрезку PQ и из точки C отметь на новой дуге точку E.",
        note: "Именно расстояние PQ переносится на вторую дугу и задает положение точки E.",
      },
      {
    ...
    draw(stepIndex) {
      ...
      if (stepIndex >= 2) {
        ...
        layers.push(drawSegment(P, Q, "line-soft"));
        layers.push(drawArc(C, chord, angle - 0.65, angle + 0.2, "circle-soft"));
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Выбери любой удобный раствор циркуля и, не меняя его, проведи две дуги с центрами в P и Q. Пусть они пересекутся в точке R внутри угла.",
  ...
  },
  {
    ...
    proof: "Точка A равноудалена от B и C (AB = AC по построению), а точка D тоже равноудалена от B и C (DB = DC по построению). Обе точки лежат на серединном перпендикуляре отрезка BC, поэтому прямая AD и есть этот перпендикуляр.",
